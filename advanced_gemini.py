...
spoken_names = set()
last_recognized = ""
gemini_active = False  #  Gemini mode initially off
...
def listen_command():
    ...
    while True:
        try:
            ...
            if "gemi start" in query:
                gemini_active = True
                speak("Gemini is now listening.")
                print("Gemini activated")

            elif "gemi stop" in query:
                gemini_active = False
                speak("Gemini has stopped listening.")
            ...
            elif gemini_active and any(keyword in query for keyword in ["what is", "tell me", "explain", "define"]):
                response = ask_gemini(query)
                print("Gemini:", response)
                speak(response[:300])  # Limit long responses
...
while True:
    ...
    # ---------- Display ----------
    cv2.imshow("Face Recognition + YOLOv5 + Gemini", frame)
